...
import { X, FileText, Image, Video, AudioLines, Languages, Wand2, Sparkles, Loader2 } from 'lucide-react';
...
const ContentGenerator: React.FC<ContentGeneratorProps> = ({ onClose }) => {
  ...
  const handleAddToCanvas = () => {
    ...
    if (contentType === 'text' && generatedContent) {
      // Create text node
      const nodeId = addNode(generatedContent, centerPosition, true, 'text');
        ...
        <>
          ...
          <CardContent className="pt-2">
            {contentType === 'text' && generatedContent && (
              <div className="border rounded-md p-3 bg-muted/30 max-h-60 overflow-y-auto text-sm whitespace-pre-line">
                {generatedContent}
              </div>
            )}
          ...
          <CardFooter>
            <Button 
              ...
              disabled={contentType === 'text' ? !generatedContent : !imageUrl}
              className="w-full"
            >
              Add to Canvas
            </Button>
